...
namespace Everest.Exceptions
{
	public class ExceptionHandler : IExceptionHandler
	{
		...
		public async Task HandleAsync(HttpContext context, Exception ex)
		{
			...
			Logger.LogError(ex, $"{context.Id} - {new { ExceptionMessage = ex.Message }}");
		...
		public Func<HttpContext, Exception, Task> OnExceptionAsync { get; set; } = async (context, ex) =>
		{
			...
			await context.Response.WriteAsync($"Failed to process request: {context.Request.Description}{Environment.NewLine}{ex}");
		};
	}
}